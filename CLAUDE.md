...
| **The real Airfield Herrenteich dataset** (DWG-measured hangar + published-spec, second-source-verified fleet incl. a folded Stemme S10 + a valid all-8 `layout.yaml`) | [`examples/herrenteich/`](examples/herrenteich/README.md) — real hangar + layout/scenario; its aircraft are the shared central `data/catalog/` entries since #595 (no per-world duplication; `cessna_150` is the only synthetic placeholder — `fuji` is the permanent low-wing occupant added in #657, published-spec envelope) |
...
`.github/dependabot.yml` is **grouped** since #973/#975 — pip `/`, npm `/viewer`, github-actions `/`, plus a **second github-actions entry for `/.github/actions/setup-python-env`** (one entry per composite-action directory; the scan does not recurse). Full rationale lives in the file's own comments; what matters here:
...
- **⚠ pip updates for *pyproject-declared* deps are blocked (#980).** Every scheduled `pip in /.` run since 2026-06-02 fails **partially**: it opens PRs for the `.in`-sourced roots (setuptools/wheel/build/pip-tools/atheris — #962/#963/#965 as late as 2026-08-17) and *then* dies with `RuntimeError: No files have changed!` on the first pyproject-declared dep. A dep with an already-satisfied `>=` floor in `pyproject.toml`, pinned only in `requirements-dev.txt` (compiled *from* pyproject), leaves the updater no manifest to edit. So `pytest`/`ruff`/`mypy`/`types-*`/`hypothesis`/`matplotlib` — **and the pillow security updates** — never get a PR. **A steady stream of pip PRs is NOT evidence pip is healthy.**
- **A valid config is not a working one.** A `Dependabot config file validation` check runs on any PR touching `dependabot.yml` — worth having, since an invalid config silently disables *all* updates including security — yet it passed on #975 while the pip entry was crashing. Run failures never appear as PR checks — look at the `event=dynamic` Actions runs (`pip in /. - Update #NNN`) or the repo's `network/updates` Dependabot page.
...
- **⚠ The real Herrenteich all-8 is statically valid but does NOT fully auto-tow-route yet (root-caused 2026-06-26; see [`docs/spikes/herrenteich-all8-tow-routing-rootcause.md`](docs/spikes/herrenteich-all8-tow-routing-rootcause.md)).** The club parks all 8 daily by monotone fill (never moving a parked plane), so a valid order provably exists — but `solve --render-paths` / `view` can't route the whole fill. **Factor 1 — FIXED (#842; the dominant cause, ~95% confidence):** `scheibe_falke` is a real LOW-wing glider whose 18 m wing was modelled in the HIGH-wing z-layer (`z[1.9,2.1]`), manufacturing a phantom wing-vs-wing collision when a high-winger towed past the parked Scheibe; the fix is a thin keep-out band `z[1.72,1.78]` (float-safe under both the herrenteich 0.15 m and the synthetic 0.20 m clearance, since the catalog is **shared**). **Factor 2 — RESOLVED, #844 CLOSED (2026-06-27 — documented manual-insertion case); GROUNDED (2026-06-26; see [`docs/spikes/herrenteich-fk9-cessna-lateral-shuffle.md`](docs/spikes/herrenteich-fk9-cessna-lateral-shuffle.md)):** the residual blocker is the `fk9_mkii`↔`cessna_140` front-door corridor — two *genuine* high-wingers that mutually space-exhaust at the deployed 0.5 m / 15° tow grid (no monotone order places both). A witness-first probe grounds it as a **search-*efficiency*** problem: a feasible **own-gear** path provably **exists** (own-gear A\* at 0.25 m/10° finds a no-carts path while the deployed 0.5 m/15° grid finds none), so the deep pivot+forward "parallel-park" shuffle is real — the coarse grid just can't represent it and finer grid finds it too slowly to ship. The **resolution hypothesis is vindicated**; carts route the pair cheaply (the **diagnostic** isolating *lateral displacement*, **not** the fix — the club hand-shuffles on own gear, so `on_carts: true` would be unfaithful). The all-8 has a **second** blocker (husky ordering). **⚠ Search-guidance fixes are REFUTED (2026-06-27, #840 NO-GO):** both the analytic parallel-park macro AND an exact heading-aware backward-SE(2) cost-to-go heuristic were built and measured — the heading-aware heuristic found the path in **108 991** expansions vs the deployed position-only heuristic's **96 949** (0.89×, ~12% *worse*). The ~97 k expansions are an **intrinsic near-C\* A\* plateau** (completeness forces expanding every `f*≤C*` state), so **no heuristic-class method — deterministic field or learned guidance — shrinks it**. The fk9↔cessna pair is therefore a **documented manual-insertion case** (the club hand-shuffles it on own gear; cache-the-witness is brittle/unfaithful, rejected). **#844 is CLOSED (2026-06-27)** — both separable follow-ups are resolved: **(a)** husky front-cluster entry-**ordering** — **gate RAN 2026-06-27 → NO-GO for *routing* → KILLED** (forcing husky-before-`wild_thing` proves husky is *pure* ordering and reaches the real fk9↔cessna wall, but that pair is **grid-geometry-locked** — each exhausts the 8 k per-plane cap, not merely the global budget — so ordering is a diagnostic/partial-fill win, **not** an all-8 routing fix, and shipping a `_place_rest` ordering heuristic would perturb the ADR-0003 determinism contract + `scene.py` whole-fill timeline order + a perf baseline for a non-routing gain — not worth it; see the lateral-shuffle spike's "Husky-ordering gate") and **(b)** a parked **continuous-trajectory-optimization** spike — its determinism-first **Gate 0 returned NO-GO (dominated)** (2026-06-27, [`docs/spikes/herrenteich-fk9-cessna-trajopt-determinism-precheck.md`](docs/spikes/herrenteich-fk9-cessna-trajopt-determinism-precheck.md)). The architectural relaxation that would actually route dense fleets — **lift the monotone-fill constraint** — plus the only surviving method class (a re-chartered *generalize-to-arbitrary-nooks* continuous-optimization bet) live on **#667** (shuffle-aware tow routing). Numbers + the full refutation are in the spike doc — don't re-investigate the heuristic/guidance path, the chartered-pair traj-opt path, *or* husky-ordering as an all-8 *routing* fix (all settled NO-GO). **#667 rung program COMPLETE (all five rungs A–E merged by 2026-06-29; epic closed-as-completed — the dense-all-8 goal itself stays unmet): the relaxation shipped as a 5-rung program** (A Stage-0 gliders · B bench tripwire · C reverse read-only diagnostic · D multi-leg data model · E move-aside core; spec [`docs/superpowers/specs/2026-06-27-667-shuffle-aware-tow-routing-design.md`](docs/superpowers/specs/2026-06-27-667-shuffle-aware-tow-routing-design.md)). **Rung A MERGED (#859):** the dolly gliders (Scheibe + Stemme) are `hand_placed: true` in `layout.yaml`/`layout_today.yaml` (and Stemme alone in `layout_full.yaml`, where Scheibe is parked outside; fill-planner keep-outs, not tow-routed) and the loader now parses the `hand_placed` placement key + rejects unknown placement keys. Move-aside (Rung E) is a **deterministic** relaxation — chosen ahead of, and complementary to, the continuous-optimization bet above — and the only reachability-adding rung; it may still not seat fk9↔cessna. **Rung B MERGED (#862):** a bench routing-ceiling baseline routes the known-valid witness layouts directly (`plan_fill`, skip solve, `placement_s=0`) — both bail budget-exhausted (deepest-unplaced `zlin_savage`) at the 8 k cap (`docs/spikes/herrenteich-routing-ceiling-baseline.md`). **Rung C MERGED (#864):** a read-only reverse-teardown probe (`reverse_teardown_probe`, towplanner.py — generalises `egress_first_conflict` into a whole-fill greedy peel; complete by egress-monotonicity, RNG-free, byte-inert) ran on the GO-free all-8 `layout.yaml` → **STUCK**: only `ctsl` extracts, leaving a **5-body mutually-blocking core** (`husky`/`cessna`/`fk9`/`wild_thing`/`zlin`), so **no monotone tow order exists** at the deployed grid + 8 k budget — the reverse-direction confirmation of the Rung-B wall, and the core is *broader* than the documented fk9↔cessna pair. Teardown only removes, never relocates, so it shares the forward fill's ceiling ⇒ a reverse-only writer AND search-guidance both add nothing (re-confirms #840/#844 NO-GO from a third angle); **move-aside (Rung E) is the indicated lever** (`docs/spikes/herrenteich-reverse-teardown-probe.md`). `layout_today.yaml` is confounded (its 3 fixed ground objects door-congest every aircraft) and discarded. **Rung D MERGED (#866):** the byte-identical **multi-leg data-model seam** — `Move.leg_index: int = 0` (additive trailing field), multiple `Move`s per `plane_id`, an optional scene/v2 `timeline.segments[].leg_index` emitted **only** for a multi-leg body (the byte-identity hinge — single-leg bodies emit no key), and the viewer (`segByPlane` → `SegmentData[]` arrays + rebuilt `viewer.js`) animating each leg / resting a body at its staging pose between legs. As of Rung D no producer emitted >1 leg (Rung E below supplies the first), so every plan/scene/render/animation was byte-identical (ADR-0003); `_timeline` lays a body's legs end-to-end. The `MovesPlan` aggregate leg-index well-formedness invariant is **deferred to Rung E** (it is not inert today — `ml/infer.py` + tests legitimately build two same-`plane_id` default-index moves). **Rung E MERGED (#869, Closes #868 — 5/5 sub-issues closed; #667 program COMPLETE):** the move-aside repair core in `_place_rest` ships as a **byte-identical capability seam** (ADR-0003), not a behavior change — a **two-phase order search**: phase 1 is today's non-displacing backtracking DFS; **phase 2** (relocate a parked body to an apron-out staging pose, route the stuck body past it, then return it — depth-1, displace-deepest-first, verify-before-displace) engages **only** on an in-budget phase-1 deadlock with `apron_depth_m>0` and a positive displacement cap, so every prior plan/scene/render/animation is byte-identical; the Rung-D-deferred F1 `MovesPlan` aggregate leg-index invariant (`__post_init__`) shipped here. It does **NOT** crack the dense all-8: that fill is **budget-bound** — phase 1 bails on `zlin_savage` at the 8 k global expansion cap *before* phase 2 can engage (re-confirmed with `--apron-depth auto`) — so `fk9_mkii`↔`cessna_140` stays the **documented manual-insertion case** and the dense all-8 route remains out of reach without unshippable finer-grid/bigger-budget or a fundamentally cheaper motion search, **not** more move-aside depth. ★ A forced *small* (2–3-body) move-aside is provably impossible (a monotone fill seats any plane first into the empty hangar, so a small deadlock is a symmetric mutual block whose displaced body can't return for the same reason; ~480-config search → 0 hits); depth-1 helps only ≥5-body cyclic cores like Rung C's.

...
4. Convert each finding into a **review thread on the diff** (via `gh pr review` line comments / `gh api .../pulls/<n>/comments`). Findings never live only in chat. **Anchor gotcha:** `POST .../pulls/<n>/reviews` resolves each comment's `line` against the **current** diff (HEAD), not the reviewed commit's blob — so if you already pushed a fix that moved/rewrote that line, the old line `422`s (`Line could not be resolved`). Post threads **before** pushing fixes, or anchor to the HEAD sha + the fix's **new** line number (threads then show "outdated").

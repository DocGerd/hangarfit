...
- **⚠ The real Herrenteich all-8 is statically valid but does NOT fully auto-tow-route yet (root-caused 2026-06-26; see [`docs/spikes/herrenteich-all8-tow-routing-rootcause.md`](docs/spikes/herrenteich-all8-tow-routing-rootcause.md)).** The club parks all 8 daily by monotone fill (never moving a parked plane), so a valid order provably exists — but `solve --render-paths` / `view` can't route the whole fill. **Factor 1 — FIXED (#842, ~95% of the gap):** `scheibe_falke` is a real LOW-wing glider whose 18 m wing was modelled in the HIGH-wing z-layer (`z[1.9,2.1]`), manufacturing a phantom wing-vs-wing collision when a high-winger towed past the parked Scheibe; the fix is a thin keep-out band `z[1.72,1.78]` (float-safe under both the herrenteich 0.15 m and the synthetic 0.20 m clearance, since the catalog is **shared**). **Factor 2 — OPEN (#844):** with Factor 1 fixed the residual blocker is the `fk9_mkii`↔`cessna_140` front-door corridor — two *genuine* high-wingers that mutually space-exhaust at the deployed 0.5 m / 15° tow grid (no monotone order places both). It is NOT turn-radius (all 8 are R=0 pivot), NOT wing-model coarseness (taper refuted), NOT the Scheibe (removing it doesn't help); search resolution/guidance is the prime suspect but a finer grid is too slow to confirm. #840 is re-scoped to it (learned/guided motion). The discarded hypotheses are tabled in the spike doc — don't re-investigate.

...
**`gh` in the review loop vs the git-guard hook.** The `~/.claude/hooks/guard-destructive-git.sh` PreToolUse hook fires on any Bash command that invokes `git` and **denies** when the command string holds **both** ` push ` and a bare `-f`/`--force` — matched *anywhere* in the string, so tokens inside quoted text (a `gh` body) or a *chained* command count. So `git push && gh api … -f …`, or a `git`-containing compound whose quoted body text mentions both "push" and "-f", is wrongly blocked as a force-push; a **standalone** `gh api -f` (no `git` in the command) is fine. Fix: run `gh` calls that carry `-f` (or a body that mentions "push"+"-f") in their **own** Bash call with no `git` alongside, and pass `gh` bodies via `--body-file`. (`-F`/`--field` is **not** a safe swap for `-f` — it type-coerces all-digit/`true`/`false`/`@…` values; the string-preserving long form of `-f` is `--raw-field`.)

...
- **Local serve backend** (`src/hangarfit/server.py`, `hangarfit serve`): a stdlib `http.server` bound to `127.0.0.1` **only** (no `--host`), guarded by a `Host`-header check, that serves the `--edit` editor (`GET /`), re-solves user-supplied scenario YAML (`POST /solve`), and converts dragged floor poses to pins (`POST /convert`). No web-framework dependency; the offline single-file export is unaffected ([ADR-0030](docs/adr/0030-hangarfit-serve-local-backend.md)). Reports of loopback-bind bypass, `Host`-header-guard bypass, or request-parsing resource exhaustion are in scope.

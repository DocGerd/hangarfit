# Spike: Tow-path planning — collision-free entry sequence for the empty-hangar fill case (rearrangement deferred to v2)
...
**Alternatives.** **Greedy back-first (deepest target first)** / Greedy peel-back (rearrangement-style) / backwards-from-goal (warehouse search) / A* over partial layouts / classical planner (PDDL) / user-supplied order.
...
**Why.** Empty-hangar fill has a natural ordering: deeper slots first, because shallower slots become obstacles for anything that needs to reach the back. *Greedy peel-back* is the dual algorithm for the rearrangement case (iteratively pull out the topmost obstructing plane until the target slot is exposed) — it presumes a non-empty starting layout to peel from, which the empty-hangar-fill scope doesn't have, so it doesn't apply here and is filed as v2 work along with the rearrangement scenario. *Backwards-from-goal* is the classical warehouse-search variant that reasons over partial layouts; it would work but is overkill here because the goal layout is already known and we don't need to search over intermediate states. User-supplied order was the v1 in the verifier framing PR #186 chose; here the tool computes the order, which removes one layer of authoring friction. *A\* over orderings* is overkill at fleet sizes ≤ 12 and adds an exponential state space we have no evidence of needing. *PDDL* would force an external dependency we have no other use for.

**Order-retry on Dubins collision.** Greedy back-first is not provably collision-free for every target layout — two same-depth planes whose Dubins arcs cross is a real failure mode. The mitigation is a bounded retry loop: when plane *P*'s Dubins path collides with an already-placed plane, swap *P* with the next-feasible plane in the order and re-try. After *K* failed swaps, fail with a structured error pointing at the offending plane and recommending the user adjust the scenario (or, in v2, RRT-Connect resolves it). *K* is an implementation detail of issue #5 — expected to scale with `n_planes` (a fixed multiple like `2 * n_planes` keeps the retry budget linear and avoids combinatorial blow-up).

**Determinism.** The v1 towplanner is **deterministic by construction**: the back-first ordering is a total deterministic sort with a tie-break on `plane_id`; the Dubins primitive is closed-form (no RNG); the order-retry swap rule is deterministic (always pick the next-feasible plane by the same sort order). Given the same `(target_layout, fleet)` it produces the same `MovesPlan` bit-identically. This preserves [ADR-0003](../adr/0003-rr-mc-solver-algorithm.md)'s seeded-reproducibility contract end-to-end through the bundled `(Layout, MovesPlan)` output of `solve` — same seed, same RR-MC output, same towplanner output, same bytes. v2 RRT-Connect breaks this contract and will need its own determinism story (probably its own seed parameter, distinct from the RR-MC seed).
...
**Why.** The collision oracle (`collisions.check` in `src/hangarfit/collisions.py`) already exists, already handles every existing rule (parts overlap, hangar bounds, bay intrusion), and already respects the parts-model granularity from [ADR-0001](../adr/0001-aircraft-parts-model.md). Sampling is the smallest delta to the codebase — the verifier becomes "iterate samples along the Dubins arc, call check, accumulate any conflict." Sweep-volume would need a new collision primitive (no `Layout`-shaped input for the existing oracle), and CCD would need yet another. Both are credible v2 upgrades if sampling's grazing-miss failure mode ever materially bites; neither is justified day-one.

Why 0.05 m / 1° is defensible as a starting step: the smallest part dimensions in `data/fleet.yaml` are fuselage widths on the order of 0.4–0.8 m (twice the half-width `fuselage_attach_y_m`), so a 5 cm step is roughly 1/8 to 1/16 of the smallest part. Combined with slow tow speeds (no momentum to worry about), the worst plausible miss is a part-thickness shave that real operators would correct visually. The step is configurable on the implementation issue (#5) so a tighter value can be wired in if real authored paths show it matters.

### Q5. Cart-mode handling — does cart change anything?

**Alternatives.** Ignore cart-mode in v1 / two-mode motion with sequence-level cart cap / cart-specific lift-walk-place / **cart = own-gear with zero-radius pivot**.

**Recommendation.** **Cart = own-gear with `turn_radius_m = 0`** (a Dubins arc collapsed to a pivot-in-place). Cart-borne planes (the three `always_cart` planes Scheibe Falke, Wild Thing, Zlin Savage, and `cart_eligible` planes when the existing single-cart-eligible rule places them on a cart) feed the same Dubins-arc machinery as own-gear planes, just with a zero radius — which mathematically becomes a pivot-in-place.
...
**Supersedes the arc42 §8 cart-kinematics framing.** [§8 Crosscutting Concepts](../architecture/08-crosscutting-concepts.md) currently states the convention as *"holonomic on carts; Dubins-path-style on own gear"* — i.e., a two-mode motion model. This recommendation collapses both onto a single Dubins primitive (carts get `turn_radius_m = 0`), which is the **opposite** convention. The §8 update bullet in implementation issue #9 must explicitly retire the holonomic-on-carts framing; failing to do so will leave §8 contradicting the implementation. ADR-0007 (issue #10) records this supersession as a decision so reviewers a year from now can see it survived scrutiny.

**Schema implication — Q5 cannot be implemented as written without a model change.** `data/fleet.yaml` sets `turn_radius_m: null` for the three `always_cart` planes today, and `src/hangarfit/models.py:179` rejects `turn_radius_m <= 0` for non-cart planes while `Aircraft.required_turn_radius_m()` (`models.py:189-203`) raises if the field is read for cart planes. To realise the recommendation, ADR-0007 picks one of:

  - **(a) Loosen the loader/validator.** Allow `turn_radius_m: 0` for `always_cart` planes; update `data/fleet.yaml` to `0` (not `null`) for the three cart planes; `required_turn_radius_m()` returns `0` instead of raising for them. **Smaller surface, cleaner contract.** Risk: a legacy reader that assumed `turn_radius_m > 0` for all planes now silently divides by zero somewhere.
  - **(b) Special-case inside the towplanner.** Keep the schema as-is; the towplanner substitutes `0` when constructing Dubins arcs for `always_cart` planes via a dedicated `aircraft.effective_turn_radius_m()` helper. **Smaller schema diff, more towplanner-internal logic.** Risk: future planners (RRT-Connect) re-implement the same substitution and drift.

(a) is the cleaner long-term shape and is the recommended path; (b) is named so the trade is visible. The risk register (#4b) captures this as a Medium-severity blocker that must land before implementation issue #5.

The honest weakness is **sequence-level cart cap**: the existing rule (at most one `cart_eligible` plane on a cart in any single layout, enforced in `Layout.__post_init__`) constrains layouts but not sequences. If the user authors a scenario with two `cart_eligible` planes that the solver places on a cart for *different* candidate layouts, the v1 towplanner happily plans both — it never re-checks the cap across moves of the same plan because each plan's target layout already satisfies the per-layout cap. This is correct under the current rule; whether the rule should be tightened to "at most one cart-eligible plane on the cart across the sequence" is a separate question that ADR-0007 (issue #10) should call out and defer.
...
class DubinsArc:
    ...
    turn_radius_m: float  # 0 for cart-borne pivot-in-place (Q5); plane's turn_radius_m otherwise
...
**`Pose` vs existing `Placement`.** `Pose` deliberately omits `plane_id` and `on_carts` — tow-path samples don't carry plane identity (the caller knows it) and don't change cart state mid-arc. Construction-from-`Placement` is a one-liner; the duplication is justified by `Placement`'s coupling to layout-level constraints we don't want sample objects to inherit. Issue #1 (dataclasses) should include the `Pose.from_placement(p: Placement) -> Pose` constructor.

**No sequence-level cart-cap field.** `MovesPlan` deliberately does not carry a cart-usage tally or `cart_id` — that's consistent with the Q5 deferral (the per-layout cap remains; the per-sequence cap is an ADR-0007 open question). Future addition is non-breaking.

When a YAML serialization is added (joint with AR spike #172's first implementation), it carries an explicit `schema_version` field — intended to be a versioned schema from day one because the AR spike will consume the same format and the two sides need a versioning hook for breaking changes.
...
| 4b | Cart-as-own-gear (`turn_radius_m = 0`) is not representable in today's schema: `data/fleet.yaml` sets `turn_radius_m: null` for the three `always_cart` planes, `models.py:179` rejects `<= 0` for non-cart planes, and `Aircraft.required_turn_radius_m()` (`models.py:189-203`) raises if read for cart planes. Q5 cannot be implemented without a loader/validator change OR a towplanner-internal substitution | Medium | ADR-0007 (issue #10) picks one of the two schema strategies in Q5; implementation issue #1 (dataclasses) is a no-op for this risk; the model change ships before issue #5 lands |
...
**Structural blockers at filing time.** Per [[feedback_encode_dependencies_in_tickets]], the soft prereq on [#145](https://github.com/DocGerd/hangarfit/issues/145) (Phase 2b inter-plane gap minimization) must be encoded as a queryable GraphQL `addBlockedBy` link from Phase 3a issues to #145, not just as prose in this doc. Issues #5 (towplanner module + order-retry) and #6 (`solve` integration) are the natural targets because they depend on the target-layout shape that #145 produces. The block-by links are added by the filing script at issue-creation time so the dependency lives in the GitHub UI and is queryable.

1. **`Pose` / `DubinsArc` / `Move` / `MovesPlan` dataclasses + tests** — pure-data foundation.
2. **Dubins arc primitive** — closed-form shortest path between two oriented poses under a minimum turn radius. ~80 lines. Standalone-tested with the standard CSC / CCC case matrix **plus a 45°-heading canary** modelled on `tests/test_geometry.py::test_heading_45_right_wingtip_in_plus_x_minus_y_quadrant`: Dubins literature uses CCW-positive-radians, hangarfit uses CW-positive-degrees compass headings (per ADR-0002), and the symmetric CSC/CCC matrix would silently pass a sign-flip regression. The canary must consume `Placement.heading_deg` through the same convention as `geometry.aircraft_parts_world`.
3. **Greedy back-first ordering** — sort target placements by `(y_m desc, x_m asc, plane_id)` with a deterministic tie-break.
4. **Sampled collision-during-motion check** — discretize each Dubins arc at a fixed step; call `collisions.check` per sample against the already-placed subset. Reuses `geometry.aircraft_parts_world` for the moving plane's parts at each sample pose. **PR will touch `geometry.py` / `collisions.py` callers, so the `geometry-invariant-guard` subagent must review** — guards the determinant-−1 sign-flip trap per ADR-0002 (standing CLAUDE.md rule, encoded here on the issue so it cannot be forgotten).
...
9. **Docs sweep: arc42 §5 + §6 + §8, ADR README, `data/fleet.yaml` comments** — per [[feedback_enumerate_doc_layers]], sweep every layer in one PR so no anchor goes stale: (a) §5: register the `towplanner` module; (b) §6: add the runtime flow for `solve` returning bundled `(Layout, MovesPlan)`; (c) §8: note that `turn_radius_m` is now consumed, the door is a towplanner-level motion gate (still a visual marker only at the `collisions.check` level), and explicitly retire the holonomic-on-carts framing per ADR-0007 (see Q5 supersession); (d) `docs/adr/README.md`: add the ADR-0007 entry; (e) `data/fleet.yaml`: update comments on the three `always_cart` planes' `turn_radius_m` placeholders to point at the towplanner now that the field becomes load-bearing.
...
- **Sequence-level cart-cap decision** — whether to tighten the existing per-layout cart cap (`Layout.__post_init__`) to a per-sequence cap. ADR-0007 deliberately defers this; the cart-lift primitive PR is the natural place to revisit it.

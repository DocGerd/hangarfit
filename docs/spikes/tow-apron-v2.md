...
1. **Bounded depth, full frontage width.** The apron is the rectangle `x ∈ [0, width_m]`, `y ∈ [−apron_depth_m, 0)`. `apron_depth_m` is a new optional `hangar.yaml` scalar; **absent ⇒ `0` ⇒ today's no-apron behaviour, reproduced byte-for-byte** (the clean migration story, mirroring the `max_carts` precedent). A site opts into an apron by setting the scalar explicitly, or by requesting a fleet-**derived** depth (`≈ max(plane length) + max(turn_radius_m)`) via an explicit `auto` value / `--apron-depth auto` — the derived depth is an opt-in convenience, **never auto-injected on absence** (that would break the byte-identity claim). **Bounded, not an unbounded half-plane** — the Hybrid-A\* free-space grid must stay finite and deterministic.
...
**Recommendation.** **(B) — a bounded rectangle `x ∈ [0, width_m]`, `y ∈ [−apron_depth_m, 0)`,** where `apron_depth_m` is a new optional `hangar.yaml` scalar (default `0`; **absent ⇒ `0` ⇒ today's no-apron behaviour, reproduced byte-for-byte**). A site opts into an apron by setting the scalar explicitly, or by requesting a fleet-**derived** depth (`≈ max(plane length) + max(turn_radius_m)`, big enough for one run-up-and-align manoeuvre and no bigger) via an explicit `auto` value; `--apron-depth N` overrides per run. The derived depth is an opt-in, **never injected on absence**.
...
**Why a stored scalar (B) over a purely-derived depth (C).** Following the [#210 `max_carts` precedent](../adr/0007-tow-path-planner-v1-scope.md) (site equipment lives on the hangar floor plan, with a CLI override for what-if exploration), the apron is a property of the *site*, not the fleet: one hangar fronts onto a big apron, another onto a tight taxiway. A `hangar.yaml` scalar (absent ⇒ `0`) plus an opt-in `auto` derived value gives both a clean byte-identical migration *and* a sane one-knob apron when wanted. The no-apron model is exactly the `apron_depth_m = 0` special case — the cleanest possible migration story (just as `max_carts`'s default `1` reproduces today's cart behaviour, `apron_depth_m`'s default `0` reproduces today's no-apron behaviour).
...
1. **A fixed, RNG-free apron-pose grid.** Generalise [`entry_poses`](../architecture/05-building-block-view.md) (today a deterministic 3 x-samples × 5 headings cone, all at `y = 0`) into a 3 × N_y × 5 grid by adding a small **fixed** set of `y`-offset samples in `[−apron_depth_m, 0]` (e.g. `{0, −apron_depth_m/2, −apron_depth_m}` — a fixed count, scaled to the stored depth). Enumerate in a **fixed order** (x-outer, y-middle, heading-inner), generalising `entry_poses`' current `(x, heading)` exact-float dedup (every pose shares `y = 0` today, so the key is a 2-tuple) to a 3-tuple `(x, y, heading)`, dropping the second occurrence. A straight-in apron-centre pose stays the always-present fallback so the frontier is never empty.

   > **Shipped deviation (#412):** the implementation excludes the `y = 0` door-line sample (uses `{−d/2, −d}`, not `{0, −d/2, −d}`) so every plane visibly slides in, and adds a 5-heading rear-entry cone, giving `3 × N_y × 10` with an apron. See [ADR-0021](../adr/0021-tow-planner-staging-apron.md) implementation notes #2–#3 for the rationale.
...
3. **Bounded, finite grid — reconciling an existing pad.** The grid heuristic (`_build_grid_heuristic`) *already* extends its free-space band a fixed `_GRID_H_Y_PAD_M = 6.0 m` south of the door (`iy_min = −round(_GRID_H_Y_PAD_M / _GRID_XY_M)`) and *already* treats the whole `y < 0` strip as free — it is only the **oracle** `_mover_motion_bounds_conflict` (Q5) that enforces the jamb gate, not the heuristic. So this change *reconciles/parameterises* that hard-coded pad with `apron_depth_m` (the pad becomes `max(6.0, apron_depth_m)`, or `apron_depth_m` supersedes it) rather than adding south-extension from scratch. Because `apron_depth_m` is bounded (Q1), the cells still extend south by a *fixed* number of rows; the geodesic precompute stays finite and reproducible. No floating apron means no undefined grid bound.
4. **No RNG, no clock.** Every new quantity (apron poses, grid bounds, the opt-in `auto`-derived depth) is a pure function of the `Hangar` + `Layout` + fleet. There is no `random`, no `time`/`Date.now`, no wall-clock. The budgets (`_MAX_EXPANSIONS`, `_MAX_FILL_EXPANSIONS`) stay deterministic *counts*; a larger space may warrant re-tuning their values, but the *count* — and therefore the output bytes — is reproducible.
...
Note the **grid heuristic** already treats `y < 0` as free space within its `_GRID_H_Y_PAD_M` south-pad (Q4.3); the jamb gate lives **only** in this **oracle** check. So the heuristic change is a bound reconciliation, while the substantive new rule — and where the `geometry-invariant-guard` scrutiny belongs — is this oracle generalisation.
...
1. **`models.py`** — add an optional `Hangar.apron_depth_m: float` (default `0`/absent ⇒ today's behaviour, byte-identical), with loader validation (`>= 0`) and an opt-in `auto` value that derives the depth for real datasets (never auto-injected on absence). Smallest additive schema change; mirrors the `max_carts` shape ([#210](../adr/0007-tow-path-planner-v1-scope.md)).
...
|---|---|---|
| **Extent** | unbounded half-plane · **bounded fixed-depth rectangle** · per-fleet-derived (no field) | **Bounded rectangle**, `hangar.yaml` scalar (default `0`) + opt-in `auto` derived depth + CLI override (Q1) |
...
| 5 | The opt-in `auto`-derived `apron_depth_m` is wrong for a real site | Low | It is overridable per-site (`hangar.yaml`) and per-run (`--apron-depth`); the default `0` (no apron) reproduces today exactly, so a bad derived value can never affect a dataset that didn't opt in |

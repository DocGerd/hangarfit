...
| `OSPS-AC-03.01` — Protect primary branch (no direct commit) | **Met** | Branch protection on `develop` (default) and `main`: pull request required, `enforce_admins=true`, `allow_force_pushes=false` — so direct commits are blocked. Zero *approving* reviews are required (a single maintainer cannot approve their own PR — see [`security-posture.md` §Code-Review](security-posture.md#code-review-score-0)), but the PR-plus-admin-enforcement is what satisfies AC-03.01's "prevent direct commits." Verified live via `gh api repos/DocGerd/hangarfit/branches/{develop,main}/protection`; rationale in [`security-posture.md` §Branch-Protection](security-posture.md#branch-protection-score-3). |

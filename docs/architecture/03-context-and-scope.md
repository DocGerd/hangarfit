...
| **Hosted / multi-user web frontend.** | A CLI plus a PNG is the right shape for the core decision (one operator, one decision, no audit trail needed yet). The `hangarfit view --edit` placement editor and the `hangarfit serve` backend are **local, single-operator, loopback-only** (`127.0.0.1`) conveniences over the same solve pipeline ([ADR-0030](../adr/0030-hangarfit-serve-local-backend.md)) — not a deployed, multi-user web service, which remains out of scope. |

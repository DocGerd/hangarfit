...
- `Scenario` gains `mover_pins: Mapping[str, Placement] = {}` (frozen/immutable like the other maps). Each value is a `Placement(plane_id=<mover_id>, x_m, y_m, heading_deg, on_carts=False, hand_placed=True)` — `hand_placed=True` is the path-less-keep-out marker the solver+planner honor (§3.3).
- **`Layout.__post_init__` guard relaxation (recon-corrected):** today `models.py:1006-1015` **hard-rejects** `hand_placed=True` on *any* `ground_object_placement` (it was an aircraft-only marker — "the planner only filters aircraft placements, so hand_placed here would be silently ignored"). PR A **generalizes** `hand_placed` to movers: relax that guard to **allow** `hand_placed=True` on a `placed_routed_mover` placement (still **reject** it on a `fixed_obstacle`), because PR A also adds the mover-side path-less handling (§3.3) so the flag is no longer silently ignored.
- `Scenario.__post_init__` invariants (new): `mover_pins.keys() ⊆ set(mover_ids)`; `mover_pins.keys()` disjoint from `region_preferences.keys()` (mutually exclusive per §2.3); each pin's `plane_id` equals its key. `GroundObject` needs no change; the only model change beyond the new `Scenario` field is the `Layout` guard relaxation above.
...
- **Path-less keep-out (recon-corrected mechanism):** the `plan_fill` mover loop (`towplanner.py:2298-2347`) today branches on `object_class` and unconditionally computes a `plan_path` for every `placed_routed_mover`; and `scene._timeline` emits path-less bodies keyed off `Move.path is None` (**not** `hand_placed`). So PR A makes a hand-placed mover path-less by:
  1. **Mover-loop branch:** in the `plan_fill` mover loop, if `gp.hand_placed` → append `Move(gp.plane_id, Pose.from_placement(gp), path=None)` and add it to `routed_mover_placements` (so later-routed movers treat it as an obstacle), skipping `plan_path`. `scene._timeline` then renders it path-less for free (`path is None`).
  2. **Aircraft keep-out:** build a distinct `aircraft_obstacle_placements` tuple (`fixed_obstacle`s plus `hand_placed` `placed_routed_mover`s, predicate `... or gp.hand_placed`) kept SEPARATE from `fixed_obstacle_placements` — not folded into it in place — so a hand-pinned mover is not double-listed once the mover loop also appends it to `routed_mover_placements` (which would trip `Layout`'s duplicate-placement guard). **Aircraft** (routed before the mover loop) consult this separate tuple, so they also route around a hand-pinned car — the faithful #667-Rung-A keep-out parity.
  It still joins the pairwise overlap loop (collision) as a static body regardless.
- **Hard-door egress preserved (recon-confirmed, no gate change):** the egress gate (`solver.py:803-813`) iterates `layout.ground_object_placements` and fires `egress_first_conflict` for every `hard_door_mover` **keyed off its resting pose** (it computes a fresh door→pose path; it never consumes the fill route). So a pinned, path-less Caddy is **still egress-gated with no gate change**, *provided* the solver materializes it into `layout.ground_object_placements` at its pin with `hard_door_mover=True` (which it does — the pinned mover is a normal `placed_routed_mover` placement, now additionally carrying `hand_placed=True` per the §3.1 guard relaxation). The gate only runs under `plan_paths=True` (pre-existing; `solver.py:762`).

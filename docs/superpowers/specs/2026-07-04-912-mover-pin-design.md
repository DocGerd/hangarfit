...
- **Path-less keep-out (recon-corrected mechanism):** the `plan_fill` mover loop (`towplanner.py:2298-2347`) today branches on `object_class` and unconditionally computes a `plan_path` for every `placed_routed_mover`; and `scene._timeline` emits path-less bodies keyed off `Move.path is None` (**not** `hand_placed`). So PR A makes a hand-placed mover path-less by:
  ...
  2. **Aircraft keep-out:** include `hand_placed` `placed_routed_mover`s in the `fixed_obstacle_placements` obstacle tuple built at `towplanner.py:1969-1973` (predicate `... or gp.hand_placed`), so **aircraft** (routed before the mover loop) also route around a hand-pinned car — the faithful #667-Rung-A keep-out parity.

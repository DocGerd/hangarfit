...
- `compute_gae(rewards, values, dones, last_value, *, gamma=0.99, lam=0.95)` → advantages + returns. **Every `done` is a true terminal here** — both set-complete *and* budget-stop genuinely end the env episode (`step()` asserts you can't step past `done`), and budget-stop is the MDP's *intrinsic* horizon at which the env emits the terminal **fraction-placed** reward (§4.5), not an artificial time-limit. So GAE zeroes the bootstrap and resets the λ-recursion at each `done`: `next_value = 0` on a `done` step, `values[t+1]` within an episode, and `last_value` (a single forward on the post-buffer live obs) only for a **non-`done`** buffer tail. No time-limit truncation bootstrap is used — adding `γ·V(s′)` at budget-stop would double-count the terminal reward the env already gave. Advantages normalized per batch.

...
- **Cost `g`** = accumulated path length. **Heuristic `h`** = `plan_dubins(pose, goal, r).length_m` — admissible (obstacles only lengthen a path) and tight ⇒ fast convergence. Both closed-form, RNG-free.

...
class TestCheckHappyPath:
    ...
    def test_default_example_layout_is_valid(self, capsys):
        ...
        ``hangarfit check examples/layouts/example.yaml`` is the first thing a new
        ...
        """
        exit_code = main(["check", str(REPO_ROOT / "examples" / "layouts" / "example.yaml")])
        assert exit_code == 0
        captured = capsys.readouterr()
        assert captured.out.strip() == "valid"
        assert captured.err == ""
    ...
    def test_default_example_invalid_layout_lists_conflicts(self, capsys):
        """The companion ``examples/layouts/example_invalid.yaml`` must exit 1 and
        produce at least one conflict — it's the demo for the red-overlay
        rendering and the JSON conflicts list."""
        exit_code = main(
            ["check", str(REPO_ROOT / "examples" / "layouts" / "example_invalid.yaml")]
        )
        assert exit_code == 1
        captured = capsys.readouterr()

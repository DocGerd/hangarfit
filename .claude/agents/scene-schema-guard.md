...
`build_scene` emits `"door": {...}` only when `layout.hangar.door is not None`, otherwise omits the key. This breaks byte-identity across door/no-door scenes' key sets and violates additive-only. Verdict: FAIL — must be always-emitted and inert-when-empty (I1/I2).

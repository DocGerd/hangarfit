...
- **PR touches the `--workers` parallel-restart fan-out (#544).** Anyone edits `_run_restarts_parallel`, `_run_restart_worker`, `_parallel_eligible`, the `workers` parameter threaded through `solve()`/`_run_solve`, or the `--workers` CLI wiring. The contract here is **parallel ≡ serial, byte-for-byte** in the eligible regime: parallel runs a *fixed* `max_restarts` restarts across a spawn `ProcessPoolExecutor` and merges them **in restart-index order** (NOT completion order), reproducing the serial accumulation exactly. Confirm the equivalence survives (see the mechanisms bullet and worked Example 5). The byte-identity tests live in `tests/test_solver_parallel.py`.
...
**⚠ The cross-machine bar is *asserted, not tested* (ADR-0003 2026-06-27 amendment, #844).** Your step-4 twice-and-diff and the `tests/test_solver_canaries.py` canaries are **same-machine / same-process**; `tests/test_solver_parallel.py` adds cross-*process* (still same-machine) byte-identity. None is a cross-*machine* golden, so a green diff proves **same-machine** reproducibility, NOT the cross-machine contract — don't report it as cross-machine proof. Two corollaries you already rely on: (1) the `set()`-ordering leak (scenario below) is exposed by `PYTHONHASHSEED=random` + the canary suite, not by a single same-process diff; (2) the tow planner's `MovesPlan` floats are closed-form but **transcendental-heavy** — `math.sin/cos/atan2/acos` in the Reeds–Shepp `_rs_lsl…_rs_lrslr` solvers (`_RS_BASE_SOLVERS`), the Dubins `_lsl…_lrl` word solvers, and the `_plan_cart` pivots — and IEEE-754 pins `sqrt`/arithmetic but **not** these, so cross-machine tow-plan identity rests on an unstated `libm` bit-stability assumption. A documented coverage caveat, not a known live divergence: treat the contract as robustly **same-machine** and *presumptively* cross-machine. See ADR-0003's 2026-06-27 amendment.
...
- **`--workers` parallel restarts ≡ serial (#544 / ADR-0003 2026-06-09 amendment).** `_parallel_eligible(search, workers)` is the gate: parallel restarts are byte-identical to serial **only** when `workers > 1` AND `search.max_restarts is not None` AND `search.spread` AND `search.spread_stall_restarts is None` — i.e. the completion-order-dependent early-exit gates (the `spread=False` first-valid exit, the spread-stall exit) are all OFF, so serial would *also* run the full fixed `max_restarts` count. For any other config the caller MUST run serial (worker count then never changes the result). The equivalence rests on two things that must survive any fan-out edit: (a) each restart is a **pure function of its index** — `_restart_rng` keys a fresh `Random` on `(resolved_seed, restart_index)`, so a worker reproduces serial restart *i* exactly regardless of which process runs it; (b) `_run_restarts_parallel` returns outputs **in restart-index order** and the caller merges them in that order, reproducing serial accumulation byte-for-byte. Weakening any clause of `_parallel_eligible` (e.g. allowing `spread=False` parallel), merging in completion order, or making a restart depend on anything but its own index is a FAIL.
...
### Example 5 — parallel fan-out merges in completion order (FAIL)

A PR "speeds up" `_run_restarts_parallel` by merging each worker's output as it finishes instead of buffering into an index-ordered list:

```python
# before — buffer, then merge in restart-index order
results: list[_RestartOutput | None] = [None] * n_restarts
for future in concurrent.futures.as_completed(future_to_index):
    results[future_to_index[future]] = future.result()
# (caller folds `results` in index order)
# after — merge as each future completes
for future in concurrent.futures.as_completed(future_to_index):
    _merge_restart(future.result(), pool, ...)   # ← completion order
```

The selection sort key ends in `restart_index`, so the *selected* layout is order-robust — BUT `best_partial_layout` accumulation and any near-ULP-tie pool insertion now depend on which worker finished first, which varies run-to-run and machine-to-machine. **`tests/test_solver_parallel.py::test_parallel_byte_identical_*` (esp. the best-partial-under-exhaustion case) goes red; the serial path is unchanged so the regression is purely in the parallel branch.** Note the twice-and-diff in step 4 (which runs `--no-spread`, i.e. the *serial* path) will NOT catch this — parallel eligibility requires spread ON, so you must run the `test_solver_parallel.py` suite (step 5) to see it. Verdict: FAIL — restore the index-ordered buffer; the merge must reproduce serial accumulation regardless of completion order.

...
| `skills/` | committed | Project skills invoked as `/<name>`: `new-fixture`, `new-adr`, `release-prep`, `release-cut`, `trim-memory`, `ml-ab` (the #607 4c-ii control-vs-treatment basin-escape A/B), and `feature-pr` (the per-PR GitFlow arc — draft PR → delegated review → thread-per-finding → ready). |

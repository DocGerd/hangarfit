...
| `agents/` | committed | Project-specific review subagents: `determinism-guard` (solver/towplanner byte-identity), `geometry-invariant-guard` (the coordinate sign-flip trap), `ml-rl-guard` (the `ml/` RL invariants — seeding/reproducibility, the 4c-ii knob default-neutrality, product-checker validity per #694, the numeric silent-failure guards), and `scene-schema-guard` (the `scene/v2` JSON-seam contract — byte-identity, the Python-owned transform, `scene.py`↔`scene-contract.ts` key parity, viewer-compare layering). |
| `skills/` | committed | Project skills invoked as `/<name>`: `new-fixture`, `new-adr`, `release-prep`, `release-cut`, `trim-memory`, `ml-ab` (the #607 4c-ii control-vs-treatment basin-escape A/B), and `feature-pr` (the per-PR GitFlow arc — draft PR → delegated review → thread-per-finding → ready; confirmation-gated, never merges). |
